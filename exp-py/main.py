...
def process_single_search_rank(args):
    ...
    return [search_rank, temp_complexity, temp_error, temp_count, temp_error_document]
...
if __name__ == "__main__":
    ...
    if USE_MULTIPROCESSING:
        ...
        if USE_MULTIPROCESSING:
            ...
            sample_complexity = [result for result in results]
    ...
    if not USE_MULTIPROCESSING:
        ...
        for search_rank in range(default_search_rank, default_search_rank_ub, 1):
            temp_complexity, temp_error, temp_count = sample_size_experiment(measurements,
    ...
    for data in sample_complexity:
        print(f"Search rank {data[0]}: Sample complexity {data[1]}, Error {data[2]}, Iterations {data[3]}")
    ...
    errors_documents = [data[4] for data in sample_complexity]
    # # Create the plot
    ...
    plt.figure(figsize=(10, 6))
    for rank, error_doc in zip(search_ranks, errors_documents):
        plt.plot(range(len(error_doc)), error_doc, label=f'Search Rank {rank}')
    plt.xlabel('Iterations (in tens)', fontsize=12)
    plt.ylabel('Error', fontsize=12)
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.title('Error vs Iterations for Different Search Ranks', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.show()
...
#Create the plot for error vs iterations
